...
try:
 ...
 checks_file=work/'checks.json'
 checks_file.write_text(json.dumps([[sys.executable,'-c','compile(open("hello.py").read(), "hello.py", "exec")']]))
 ...
 fail('Push without passing checks refused','push','--task',task)
 bad_checks=work/'bad-checks.json';bad_checks.write_text(json.dumps([[sys.executable,'-c','raise SystemExit(1)']]))
 fail('Failing checks block upload','check','--task',task,'--commands-file',bad_checks)
 command('check','--task',task,'--commands-file',checks_file)
 ...
 first=command('push','--task',task,'--wip','--send');check(first['patchset']==1,'Create Gerrit patch set 1')
 check(command('status','--task',task)['remote']['work_in_progress'],'Intermediate change is WIP')
 check(command('ready','--task',task)['mark_ready'],'Readiness preview for tested WIP revision')
 check(command('ready','--task',task,'--send')['ready'],'Readiness-only transition confirmed')
 ...
 command('check','--task',stale,'--commands-file',checks_file)
 ...
 resumed=work/'resumed';command('resume',first['link'],'--task',resumed)
 check(git('status','--porcelain')==before and (repo/'local.txt').read_text()=='preserve me','Independent remote clone leaves unrelated dirty checkout untouched')
 source=resumed/'repo'
 (source/'hello.py').write_text('def hello(name="world"):\n    return "Hello, " + name\n')
 command('commit','--task',resumed,'--path','hello.py','--message-file',message)
 command('check','--task',resumed,'--commands-file',checks_file)
 third=command('push','--task',resumed,'--ready','--send');check(third['patchset']==3 and third['number']==first['number'],'Resume link uploads third patch set')
 check(command('status','--task',resumed)['remote']['current_revision']==third['commit'],'Status confirms server SHA')
 check(not command('status','--task',resumed)['remote'].get('work_in_progress',False),'Final patch set is ready')
 reviewer_name='requested-'+uuid.uuid4().hex[:10]
 reviewer=admin.call('PUT','/accounts/'+reviewer_name,{'name':'Requested Reviewer','email':reviewer_name+'@example.com'})
 preview=command('reviewers','--task',resumed,'--reviewer',reviewer_name)
 check(preview['dry_run'] and reviewer['_account_id'] in preview['add_reviewer_ids'],'Resolve requested reviewer in preview')
 assigned=command('reviewers','--task',resumed,'--reviewer',reviewer_name,'--send')
 check(assigned['confirmed'],'Requested reviewer assignment confirmed')
 check(command('reviewers','--task',resumed,'--reviewer',reviewer_name)['add_reviewer_ids']==[],'Existing reviewers are not re-added')
 ...
 command('commit','--task',resumed,'--path','hello.py','--message-file',message)
 command('check','--task',resumed,'--commands-file',checks_file)
 # Advance target branch independently; push must require a rebase and fresh checks.
 advance=work/'advance';client.git('clone',https+'/a/'+project,str(advance))
 client.git('config','user.name','Review Bot Test',cwd=advance);client.git('config','user.email',user+'@example.com',cwd=advance)
 (advance/'upstream.txt').write_text('upstream update\n')
 client.git('add','upstream.txt',cwd=advance);client.git('commit','-m','Advance main',cwd=advance);client.git('push','origin','HEAD:main',cwd=advance)
 fail('Advanced target branch blocks push','push','--task',resumed)
 old_id=r.load(resumed/'task.json')['change_id'];command('sync','--task',resumed)
 check((source/'upstream.txt').exists() and r.load(resumed/'task.json')['change_id']==old_id,'Sync rebases onto latest branch preserving Change-Id')
 fail('Rebase invalidates passing checks','push','--task',resumed)
 command('check','--task',resumed,'--commands-file',checks_file)
 check(command('push','--task',resumed)['dry_run'],'Fresh checks permit preview after rebase')
 ...
 conflict_head=r.load(resumed/'task.json')['head']
 (advance/'hello.py').write_text('upstream conflict\n')
 client.git('add','hello.py',cwd=advance);client.git('commit','-m','Conflicting upstream edit',cwd=advance);client.git('push','origin','HEAD:main',cwd=advance)
 fail('Rebase conflict reports failure','sync','--task',resumed)
 check(git('rev-parse','HEAD')==conflict_head and not git('status','--porcelain'),'Conflicted rebase abort preserves original clean commit')
